...
"""
10 epochs, hidden size (200, cube) -> 81.31 UAS 
20 epochs, hidden size [(200, cube), (200, relu)] -> 86.89 UAS 
"""
...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        h = tfp.layers.dense_variational.DenseFlipout(H, input_shape=(D,)) (x)
        h = tf.pow(h, 3 * tf.ones_like(h))
        h_drop = tf.layers.dropout(h, rate = 1 - self.dropout_placeholder)
        h2 = tfp.layers.dense_variational.DenseFlipout(D, input_shape=(D, ), activation=tf.nn.relu) (h_drop)
        h2_drop = tf.layers.dropout(h2, rate = 1 - self.dropout_placeholder)
        pred = tfp.layers.dense_variational.DenseFlipout(C, input_shape=(D, )) (h2_drop)
        return pred
